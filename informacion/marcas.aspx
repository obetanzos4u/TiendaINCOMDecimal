<%@ Page Language="C#" AutoEventWireup="true"   Async="true" CodeFile="marcas.aspx.cs" MasterPageFile="~/general.master" Inherits="marcas" %>


<asp:Content ID="Content2" ContentPlaceHolderID="contenido" runat="Server">
    <div>
        <div class="main_container-info_marcas">
            <div class="image-wrapper-container-info_marcas">
                <h1 class="title-info-marcas">Conoce nuestras marcas</h1>
                <div class="container-info_marcas">
                    <div class="efect-tl-info_marcas"></div>
                    <div class="element-info_marcas">
                        <img src="/img/marcas/Polymerico_marca.png" alt="Polymerico" class="img-list_marca-4">
                    </div>
                    <div class="element3-info_marcas">
                        <ul>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=top_polimerico">
                                    Registros prefabricados
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=polymerico_acc">
                                    Accesorios para registros
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="container-info_marcas">
                    <div class="efect-tl-info_marcas"></div>
                    <div class="element-info_marcas">
                        <img src="/img/marcas/IcoPlastik_marca.png" alt="IcoPlastik" class="img-list_marca-2">
                    </div>
                    <div class="element3-info_marcas">
                        <ul>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoplastik_ducto">
                                    Ductos
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoplastik_micro">
                                    Microductos
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoplastik_cople">
                                    Coples y tapones
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="container-info_marcas">
                    <div class="efect-tl-info_marcas"></div>
                    <div class="element-info_marcas">
                        <img src="/img/marcas/Metalico_marca.jpg" alt="Metalico" class="img-list_marca-3">
                    </div>
                    <div class="element3-info_marcas">
                        <ul>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=busqueda=metalico_adss">
                                    Herrajes para ADSS
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=metalico_tension">
                                    Herrajes tensión
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=metalico_suspension">
                                    Herrajes suspensión
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=metalico_accesorios">
                                    Accesorios
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="container-info_marcas">
                    <div class="efect-tl-info_marcas"></div>
                    <div class="element-info_marcas">
                        <img src="/img/marcas/Tuiko_marca.jpg" alt="Tuliko" class="img-list_marca-5">
                    </div>
                    <div class="element3-info_marcas">
                        <ul>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=tuliko_fleje">
                                    Flejes y hebillas
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=tuliko_ext">
                                    Planta externa
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=tuliko_herramienta">
                                    Herramientas
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=tuliko_cinchos_acc">
                                    Cinchos y accesorios
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=tuliko_miscelaneos">
                                    Misceláneos
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div id="mobile-container-info_marcas" class="container-info_marcas">
                    <div class="efect-tl-info_marcas"></div>
                    <div class="element-info_marcas">
                        <img src="/img/marcas/Icoptiks_marca.jpg" alt="Icoptiks" class="img-list_marca-1">
                    </div>
                    <div class="element3-info_marcas">
                        <ul>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_cable_fo">
                                    Cable de fibra óptica
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_herramienta">
                                    Herramientas y accesorios
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_equipos">
                                    Equipos empalme y medición
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_splitters">
                                    Splitters
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_conectores">
                                    Conectores y acompladores
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_atenuadores">
                                    Atenuadores
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_jumper">
                                    Jumper de fibra óptica
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_odf">
                                    Distribuidores ópticos
                                </a>
                            </li>
                            <li>
                                <a href="https://www.incom.mx/productos/buscar?busqueda=icoptiks_iden">
                                    Identificación
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>

        <h2 class="subtitle-info-marcas">En INCOM.MX encontraras marcas destacadas</h2>

        <div class="logos-container">
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Bosch_marca.png" alt="Marca Bosch" title="Bosch" class="logo-list_marca">
                </div>                
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Brady_marca.png" alt="Marca Brady" title="Brady" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/CalAm_marca.png" alt="Marca CalAm" title="CalAm" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Charofil_marca.png" alt="Marca Charofil" title="Charofil" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Condumex_marca.png" alt="Marca Condumex" title="Condumex" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/3MCorning_marca.png" alt="Marca 3M Corning" title="Corning" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Cuprum_marca.png" alt="Marca Cuprum" title="Cuprum" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/EJGrup_marca.png" alt="Marca EJGrup" title="EJGrup" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Fremco_marca.png" alt="Marca Fremco" title="Fremco" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/GMP_marca.png" alt="Marca GMP" title="GMP" class="logo-list_marca">
                </div>
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/GoldenEagle_marca.png" alt="Marca Golden Eagle" title="Golden Eagle" class="logo-list_marca">
                </div>              
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Inno_marca.png" alt="Logo 11" title="Logo 11" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Intellinet_marca.png" alt="Marca Intellinet" title="Intellinet" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Jonard_marca.png" alt="Marca Jonard" title="Jonard" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Leviton_marca.png" alt="Marca Leviton" title="Leviton" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Manhattan_marca.png" alt="Marca Manhattan" title="Manhattan" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Jaguar_marca.png" alt="Marca Jaguar" title="Jaguar" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Panduit_marca.png" alt="Marca Panduit" title="Panduit" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/PLP_marca.png" alt="Marca PLP" title="PLP" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Radiodetection_marca.png" alt="Radio Detection" title="Marca Radio Detection" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Ripley_marca.png" alt="Marca Ripley" title="Ripley" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Surtek_marca.png" alt="Marca Surtek" title="Surtek" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Trupper_marca.png" alt="Marca Trupper" title="Trupper" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Urrea_marca.jpg" alt="Marca Urrea" title="Urrea" class="logo-list_marca">
                </div>             
            </div>
            <div class="marca_wrapper">
                <div class="circle-marca-wrapper">
                    <img src="/img/marcas/Yokogawa_marca.png" alt="Marca Yokogawa" title="Marca Yokogatitle" class="logo-list_marca">
                </div>             
            </div>
        </div>
    </div>
<style>
    .main_container-info_marcas {
        width: 90%;
        margin: auto;
    }

    .title-info-marcas {
        width: 100%;
        font-size: 1.56rem;
        font-weight: 600;
    }

    .subtitle-info-marcas {
        font-size: 1.56rem;
        font-weight: 600;
    }

    .image-wrapper-container-info_marcas {
        display: inline-flex;
        flex-wrap: wrap;
        position: relative;
        overflow: hidden;
        box-sizing: border-box;
    }

    .image-wrapper-container-info_marcas.ms-edge {
        background-color: aquamarine;
    }

    .container-info_marcas {
        position: relative;
        display: flex;
        flex-direction: column;
        align-items: center;
        overflow: hidden;
        box-sizing: border-box;
        flex-basis: 20%;
    }

    .img-list_marca-1 {
        width: auto; height: 22%;        
    }

    .img-list_marca-2 {
        width: auto; height: 22%;       
    }

    .img-list_marca-3 {
        width: auto; height: 28%;     
    }

    .img-list_marca-4 {
        width: auto; height: 28%;
    }

    .img-list_marca-5 {
        width: auto; height: 26%;
    }

    .element-info_marcas {
        position: relative;
        height: 310px;
        margin-right: auto;
        margin-left: auto;
        transform-origin: top;
        transition: all 0.96s ease;
        padding-bottom: 2rem;
        display: flex;
        align-items: center;
    }

    .efect-tl-info_marcas {
        width: 100%;
        position: absolute;
        left: 0;
        right: 0;
        top: 0;
        bottom: 0;
    }

    .element2-info_marcas {
        text-align: left;
        height: 60px;
        margin-left: 20px;
        margin-right: 20px;
        border-top: 1px solid #E3E7E7;
        padding-top: 11px;
        background-color: blue;
        position: absolute;
        left: 0;
        right: 0;
        top: 70%;
    }

    .element3-info_marcas {
        display: flex;
        justify-content: center;
        height: 60%;
        margin-top: 0;
        position: absolute;
        top: 100%;
        left: 0;
        width: 100%;
        transform: translateY(80%);
        transition: all 1.4s ease;
    }

    .element3-info_marcas > ul {
        width: 70%;
        border-top: 1px solid #2525253b;
        padding-top: 6px;
        padding-left: 20px;
        margin: 0;
    }

    .element3-info_marcas > ul > li {
        text-decoration: none;
        list-style: none;
        line-height: 1.8;
        font-size: .85rem;
    }

    .element3-info_marcas > ul > li > a {
        color: #000000;
    }

    .efect-tl-info_marcas:hover ~ .element-info_marcas {
        transform: scale(0.6);
        transition-property: transform;
        transition-duration: 400ms;
        transition-timing-function: linear;
        transition-delay: 0.06s;
    }

    .efect-tl-info_marcas:hover ~ .element3-info_marcas {
        transform: translateY(-100%);
        transition-property: transform;
        transition-duration: 450ms;
        transition-timing-function: linear;
        transition-delay: 0.1;
    }

    ul.info_marcas {
        position: relative; 
        z-index: 101;
    }

    .logos-container {
        overflow: hidden;
        margin: 2rem auto 15% auto;
        display: flex;
        justify-content: center;
        flex-wrap: wrap;
    }

    .logos-list {
        margin: auto;
    }

    .marca_wrapper {
        float: left;
        margin: 20px;
        width: 15%;
    }

    .circle-marca-wrapper {
        border-radius: 50%;
        width: 95%;
        height: 60px;
        display: flex;
        justify-content: center;
        align-items: center;
    }

    .circle-marca-wrapper img {
        max-height: 100%;
    }

    .logo-list_marca {
        max-width: 100%;
        max-height: 70%;
    }

    .efect-tl-info_marcas {
        pointer-events: auto;
        z-index: 100;
    }

    .element3-info_marcas {
        pointer-events: auto;
        z-index: 1000;
    }

    @media only screen and (min-width: 1600px) {
        .marca_wrapper {
            margin: 50px !important;
        }
    }

    @media only screen and (min-width: 1000px) and (max-width: 1600px) {
        .container-info_marcas {
            flex-basis: 33%;
        }

        .marca_wrapper {
            margin: 40px !important;
        }
    }

    @media only screen and (min-width: 700px) and (max-width: 1000px) {
        .title-info-marcas {
            font-size: 1.56rem !important;
            margin-bottom: 2rem !important;
            margin: 0 auto;
        }
        
        .container-info_marcas {
            flex-basis: 50% !important;
            height: 160px !important;
            margin: 0 auto;
        }

        .element-info_marcas {
            height: 240px;
            align-items: start !important;
            margin: 0 auto;
        }

        .element3-info_marcas {
            height: 46% !important;
            top: 76% !important;
        }

        .img-list_marca-2 {
            height: 26% !important;
            width: 50%;
            margin: 0 auto;
        }

        .img-list_marca-4 {
            height: 20% !important;
            margin: 0 auto;
        }

        .img-list_marca-3 {
            height: 39% !important;
            width: 140px;
            margin: 0 auto;
        }

        .img-list_marca-1 {
            height: 15% !important;
            width: 100%;
            margin: 0 auto;
        }

        .img-list_marca-5 {
            height: 34% !important;
            margin: 0 auto;
        }
        
        #mobile-container-info_marcas {
            height: 280px !important;
        }

        .container-info_marcas:last-of-type > .element3-info_marcas {
            height: 60% !important;
        }

    }

    @media only screen and (max-width: 700px) {
        .container-info_marcas {
            flex-basis: 100%;
        }

        #mobile-container-info_marcas {
            display: flex;
            height: 310px !important;
        }

        .main_container-info_marcas {
            display: flex;
            flex-direction: column;
            margin-bottom: 15% !important;
        }

        .image-wrapper-container-info_marcas {
            order: 2;
        }

        .logos-container {
            order: 1;
        }

        .logos-container {
            margin: 5% auto !important;
        }

        .marca_wrapper {
            margin: 9px !important;
        }

        .title-info-marcas, .subtitle-info-marcas {
            font-size: 1.25rem !important;
        }

        .title-info-marcas {
            margin: 2.8rem 0 2.8rem 0 !important;
        }

        .container-info_marcas {
            flex-basis: 50%;
            height: 160px !important;
        }

        .element-info_marcas {
            height: 190px !important;
            padding-bottom: 0rem !important;
            align-items: start !important;
            margin-left: 0 !important;
        }

        .element3-info_marcas {
            height: 45% !important;
            top: 76% !important;
        }

        .img-list_marca-2 {
            height: 25% !important;
            padding-top: 8px;
        }

        .element3-info_marcas > ul {
            width: 80% !important;
        }

        .img-list_marca-3 {
            width: 80%;
            height: 34%;
        }

        #mobile-container-info_marcas {
            margin: 0 auto !important;
        }

        .container-info_marcas:last-of-type > .element3-info_marcas {
            top: 64% !important;
        }
    }

    @media only screen and (max-width: 480px) { 
    
    #mobile-container-info_marcas {
        height: 275px !important;
        margin-top: 2rem !important;
        margin-bottom: 2rem !important;
    }

    #mobile-container-info_marcas > .element3-info_marcas {
        top: 64% !important; 
    }

    .container-info_marcas {
        flex-basis: 75% !important;
        margin: auto !important;
    }

    .element3-info_marcas > ul {
        width: 60% !important;
    }

    .element3-info_marcas > ul {
        margin: 0px auto 0px 0px !important;
    }

    .element3-info_marcas > ul > li {
        line-height: 1.6;
        font-size: .7rem;
    }

    .marca_wrapper {
        width: 29% !important;
    }
    }

</style>
<script>
const element3InfoMarcasList = document.querySelectorAll(".element3-info_marcas");
const elementInfoMarcasList = document.querySelectorAll(".element-info_marcas");

for (let i = 0; i < element3InfoMarcasList.length; i++) {
    const element3InfoMarcas = element3InfoMarcasList[i];
    const elementInfoMarcas = elementInfoMarcasList[i];

    element3InfoMarcas.addEventListener("mouseover", () => {
        element3InfoMarcas.style.transform = "translateY(-100%)";
        element3InfoMarcas.style.transition = "transform 450ms linear 0.1s";
        elementInfoMarcas.style.transform = "scale(0.6)";
        elementInfoMarcas.style.transition = "transform 400ms linear";
    });

    element3InfoMarcas.addEventListener("mouseout", () => {
        element3InfoMarcas.style.transform = "";
        element3InfoMarcas.style.transition = "";
        elementInfoMarcas.style.transform = "";
        elementInfoMarcas.style.transition = "";
    });
    }

    var isEdge = window.navigator.userAgent.indexOf("Edge") > -1;
    var element = document.querySelector(".image-wrapper-container-info_marcas");

    if (isEdge) {
            element.classList.add("ms-edge");
        } 
    else {
        element.classList.add("not-ms-edge");
    }


</script>
</asp:Content>