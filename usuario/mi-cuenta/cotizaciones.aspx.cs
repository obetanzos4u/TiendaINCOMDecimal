...
{ 
    ...
    protected void cargarInfo(object sender, EventArgs e)
    {
        ...
        DataTable dtCotizaciones = new DataTable(); // obtener.obtenerCotizacionesUsuario_min(datosUsuario.email);
        ...
        if (year > 6)
        {
            ...
            dtCotizaciones = obtener.obtenerCotizacionDatos_min(datosUsuario.email, desde, hasta);
        }
        else
        {
        ...
        if(!string.IsNullOrEmpty(txt_search.Text)) {
                dtCotizaciones = obtener.obtenerCotizacionesUsuario_min(datosUsuario.email);
    ...
    protected void lvCotizaciones_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        LinkButton btn_EliminarPermanentemente = e.Item.FindControl("btn_EliminarPermanentemente") as LinkButton;
        ...
        if (!IsPostBack)
        {
            ...
            if (usuarios.userLogin().tipo_de_usuario == "usuario" && usuarios.userLogin().rango == 3)
            {

                btn_EliminarPermanentemente.Visible = true;
            }
    ...
    protected void btn_crear_cotizacion_en_blanco_Click(object sender, EventArgs e) {
      ...
      //  string script = @" $(document).ready(function(){ $('#modal_crear_cotizacion_en_blanco').modal('close');    });";
      //  ScriptManager.RegisterStartupScript(this.Page, typeof(LinkButton), "modal_crear_cotizacion_en_blanco", script, true);
    }
    ...
    protected void btn_EliminarPermanentemente_Click(object sender, EventArgs e)
    {
        LinkButton btn_EliminarPermanentemente = sender as LinkButton;
        ListViewItem lvItem = (ListViewItem)btn_EliminarPermanentemente.NamingContainer;

        Label lbl_numero_operacion = lvItem.FindControl("lbl_numero_operacion") as Label;


        string numero_operacion = lbl_numero_operacion.Text;

        var result = cotizaciones.borrarPermanentementeStatic(numero_operacion);
