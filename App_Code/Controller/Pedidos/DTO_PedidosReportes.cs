...
*/
        ...
        foreach (var año in RegistrosAños)
        {
            foreach (var mes in RegistroMeses)
            {
                ...
                decimal totalPedidosPagadosMonto = 0;
                ...
                {
                    var pedido = await PedidosEF.ObtenerPagoPedido(p.numero_operacion);
                    var pagado = await PedidosEF.obtenerPagoPedidoPagado(p.numero_operacion);
                    decimal monto = decimal.Parse(pagado.message);
                    if (pedido.result)
                    {
                        TotalPedidosPagados += 1;
                    }
                    totalPedidosPagadosMonto += monto;
                }

                DesgloseMes.TotalPedidosMes = TotalPedidosMes;
                DesgloseMes.Year = año.Key;
                DesgloseMes.Mes = mes.Key;
                //DesgloseMes.MontoTotalMesSNImpuestos = Pedidos.Where(p => p.PedidoDatos.fecha_creacion.Year == año.Key && p.PedidoDatos.fecha_creacion.Month == mes.Key).Sum(p => p.MontoProductos_MXN_Sin_Impuestos);
                DesgloseMes.MontoTotalMesSNImpuestos = totalPedidosPagadosMonto;
                DesgloseMes.MesNombre = CultureInfo.CurrentCulture.DateTimeFormat.GetMonthName(DesgloseMes.Mes).ToPascalCase(CultureInfo.CurrentCulture);
